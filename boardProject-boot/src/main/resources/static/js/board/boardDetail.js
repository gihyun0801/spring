...
// 1) 로그인한 회원 번호 준비
// --> session에서 얻어오기(session은 서버에서 관리하기때문에 JS에서 바로 얻어올방법없음)
...
document.querySelector("#boardLike").addEventListener("click", function(e){
        ...
        fetch("/board/like", {method : "POST", headers : {"Content-Type" : "application/json"}, body : JSON.stringify(obj) })
        .then(response => {

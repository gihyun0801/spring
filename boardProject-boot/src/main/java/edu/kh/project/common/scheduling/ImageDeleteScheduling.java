...
public class ImageDeleteScheduling {
	...
	*/
	...
	@Scheduled(cron = "0,0 0 1 * * *") //시계 초 단위가 0초이거나 30초일때 수행
...
//			log.info("스케쥴러 동작!");
			...
			// 참조하는 폴더에 존재하는 파일 목록 얻어오기
			 ...
			 // 두 배열을 하나로 합침 ( for 문을 한번만 돌기 위해)
			 
			 // imageArr 라는 빈 배열을 boardArr memberArr 길이 만큼의 크기로 만든다
			 ...
			 System.arraycopy(boardArr, 0, imageArr, 0, boardArr.length);
			                ...
			                //boardArr 을 복사해서 0번째 인덱스부터 복사해서 
			  				//imageArr 에 0번재 인덱스부터 넣을거다 끝까지
			 				// boardArr의 길이만큼
...
//			 log.debug("serverImageList: " + serverImageList);
			 // 2. DB 에 이미지 파일 이름만 모두 조회
